...
function enforceUploadCap(req, destPath, maxBytes = LIMITS.MAX_UPLOAD_BYTES){
  return new Promise((resolve, reject) => {
    ...
    function cleanupAndReject(err){
      ...
      req.destroy();
      out.destroy();
      // Wait for the unlink attempt (success or failure — e.g. ENOENT if
      // out.destroy() hadn't flushed anything yet) before rejecting, so a
      // caller awaiting this promise never sees the temp file mid-cleanup.
      fs.promises.unlink(destPath).catch(() => {}).finally(() => reject(err));
    }
